...
impl<V> NodeAllocator<u64, V, M> for Spy<V> {
    fn alloc_leaf_uninit(&mut self) -> NonNull<MaybeUninit<Leaf<u64, V, M>>> {
        self.arena.alloc_leaf_uninit()
    }

    fn alloc_inner_uninit(&mut self) -> NonNull<MaybeUninit<Inner<u64, V, M>>> {
        self.arena.alloc_inner_uninit()
    }
...
impl NodeAllocator<u64, u64, M> for Lopsided {
    fn alloc_leaf_uninit(&mut self) -> NonNull<MaybeUninit<Leaf<u64, u64, M>>> {
        self.leaves.alloc_leaf_uninit()
    }

    fn alloc_inner_uninit(&mut self) -> NonNull<MaybeUninit<Inner<u64, u64, M>>> {
        self.inners.alloc_inner_uninit()
    }
