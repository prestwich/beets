...
/// The uninit primitives are the pool's slot service: acquire slots,
/// return one never-written, initialize the other by hand and retire
/// it through the value method.
#[test]
fn uninit_slots_round_trip_through_the_arena() {
    use crate::NodeAllocator;

    let mut arena: Slabs<u64, u64, M> = Slabs::new();
    let a = arena.alloc_leaf_uninit();
    let b = arena.alloc_leaf_uninit();
