...
/// A box-per-node allocator has no ceiling and holds no inventory:
/// capacity is unbounded, and nothing is ever servable without asking
/// the heap.
#[test]
fn a_global_allocator_reports_no_ceiling_and_no_inventory() {
    assert_eq!(NodeAllocator::<u64, u64, M>::leaf_capacity(&System), None);
    assert_eq!(NodeAllocator::<u64, u64, M>::inner_capacity(&System), None);
    assert_eq!(NodeAllocator::<u64, u64, M>::leaf_available(&System), 0);
    assert_eq!(NodeAllocator::<u64, u64, M>::inner_available(&System), 0);
}
