...
impl<K: Key, V, const M: usize> Inner<K, V, M> {
    ...
    // The `Exhaustion = Infallible` bound: as `Leaf::splitting_insert` —
    // reached only through the reservation wrapper (or an infallible
    // allocator directly), so allocation provably cannot fail.
    pub(crate) fn splitting_insert_child<A: NodeAllocator<K, V, M, Exhaustion = Infallible>>(
        &mut self,
        partition: usize,
        sep: K,
        child: Node<K, V, M>,
    ...
    /// decision lives here.
    pub(crate) fn insert_child<A: NodeAllocator<K, V, M, Exhaustion = Infallible>>(
        &mut self,
        partition: usize,
        sep: K,
        child: Node<K, V, M>,
