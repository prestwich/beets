...
//! implementation, the global heap as the boxed-per-node alternative,
//! and the fixed-region arena ([`FixedNodes`]) for allocation-free
//! environments.
//!
...
//! callers can reserve storage fallibly before committing values to it,
//! which is how the tree's `try_` surface stays atomic on exhaustion.
...
mod fixed;
pub use fixed::{FixedNodes, NodeStorage};

mod reserve;
pub(crate) use reserve::{Reservation, Reserved};
...
mod r#trait;
pub use r#trait::{AllocError, NodeAllocator};

#[cfg(feature = "alloc")]
pub use r#trait::Global;

/// The tree's default allocator: the slab arena ([`Slabs`]) when the
/// crate has `alloc`; built core-only, the fixed arena over `'static`
/// storage ([`FixedNodes`]) — the only allocator that exists there.
/// (A TYPE default, not a value default: core-only construction still
/// goes through `try_new_in` with explicitly borrowed storage.)
#[cfg(feature = "alloc")]
pub type DefaultAllocator<K, V, const M: usize> = Slabs<K, V, M, Global>;

/// The tree's default allocator: with the crate built core-only, the
/// fixed arena over `'static` storage ([`FixedNodes`]) — the only
/// allocator that exists there.
#[cfg(not(feature = "alloc"))]
pub type DefaultAllocator<K, V, const M: usize> = FixedNodes<'static, K, V, M>;
